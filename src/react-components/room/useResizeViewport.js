...
function calculateRendererSize(canvasRect, maxResolution, isVR) {
  ...
  const convertionRatio = Math.min(maxWidth / canvasRect.width, maxHeight / canvasRect.height);

  return {
    width: Math.round(canvasRect.width * convertionRatio),
    height: Math.round(canvasRect.height * convertionRatio)
  };
}
